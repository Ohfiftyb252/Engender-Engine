...
export function buildRhythmGrid(rng: () => number, dna: DNAProfile, genre: GenreProfile, targetNotes: number, allowTriplets: boolean, totalTicks = TOTAL_TICKS): GridSlot[] {
  ...
  // Scale attempt budget with target so dense patterns can actually fill
  const maxAttempts = targetNotes * 20 + 400;

  while (slots.filter(s => s.active).length < targetNotes && attempts < maxAttempts) {
    attempts++;
