...
export function generateMelody(
  ...
  for (const slot of grid) {
    ...
    if (r < dna.melodicLeapChance) {
      // Controlled leap
      const leap = randInt(rng, 5, 12) * (rng() < 0.5 ? 1 : -1);
      ...
      // Minor 2nd tension step
      pitch = prevPitch + (rng() < 0.5 ? 1 : -1);
      pitch = Math.max(MEL_MIN, Math.min(MEL_MAX, pitch));
    } else if (rng() < dna.repetitionBias) {
      // Repeat previous pitch (hypnotic pattern)
      pitch = prevPitch;
    } else {
      // Scale walk with dark bias
      pitch = pickPitchWithBias(rng, scalePitches, allPitches);
    }

    prevPitch = pitch;

    // Staccato bias: shorter than grid slot
    const duration = Math.max(1, Math.round(slot.duration * (0.4 + rng() * 0.4)));
    const velocity = Math.max(
      40,
      Math.min(127, randInt(rng, velMin, velMax) + Math.round((rng() - 0.5) * dna.velocityVariance)),
    );
